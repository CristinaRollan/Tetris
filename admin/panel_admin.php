    ...
    <div class="container mt-5">
        ...
        <div id="dashboard" class="tab-content active mt-4">
            ...
            <form id="filter-form" method="get" class="mb-3">
                <div class="form-row">
                    <div class="col">
                        <input type="text" class="form-control" name="id" placeholder="ID" value="<?php echo isset($_GET['id']) ? htmlspecialchars($_GET['id']) : ''; ?>">
                    </div>
                    <div class="col">
                        <input type="text" class="form-control" name="nombre" placeholder="Nombre" value="<?php echo isset($_GET['nombre']) ? htmlspecialchars($_GET['nombre']) : ''; ?>">
                    </div>
                    <div class="col">
                        <input type="text" class="form-control" name="email" placeholder="Email" value="<?php echo isset($_GET['email']) ? htmlspecialchars($_GET['email']) : ''; ?>">
                    </div>
                    <div class="col">
                        <select class="form-control" name="bloqueado">
                            <option value="">Bloqueado</option>
                            <option value="1" <?php echo isset($_GET['bloqueado']) && $_GET['bloqueado'] === '1' ? 'selected' : ''; ?>>Sí</option>
                            <option value="0" <?php echo isset($_GET['bloqueado']) && $_GET['bloqueado'] === '0' ? 'selected' : ''; ?>>No</option>
                        </select>
                    </div>
                    <div class="col">
                        <select class="form-control" name="validado">
                            <option value="">Validado</option>
                            <option value="0" <?php echo isset($_GET['validado']) && $_GET['validado'] === '0' ? 'selected' : ''; ?>>Sí</option>
                            <option value="1" <?php echo isset($_GET['validado']) && $_GET['validado'] === '1' ? 'selected' : ''; ?>>No</option>
                        </select>
                    </div>
                    <div class="col">
